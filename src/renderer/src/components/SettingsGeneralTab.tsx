...
export default function SettingsGeneralTab(): React.ReactElement {
  ...
  const permissionDropdownRef = React.useRef<HTMLDivElement>(null)
  ...
  const currentPermissionMode = PERMISSION_MODES.find(m => m.id === settings.workspaceMode) || PERMISSION_MODES[1]
  ...
  return (
    <div className="settings-general-wrapper">
      ...
      {/* 6. 安全与权限 */}
      <div className="settings-general-card">
        <div className="settings-general-row" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '16px' }}>
          <div>
          ...
          <div className="permission-dropdown-container" ref={permissionDropdownRef}>
            ...
            >
               <div className="permission-dropdown-trigger-icon">{getPermissionIcon(currentPermissionMode.id)}</div>
               <div className="permission-dropdown-trigger-text">
                  <div className="permission-dropdown-trigger-title">{currentPermissionMode.title}</div>
                  <div className="permission-dropdown-trigger-subtitle">{currentPermissionMode.subtitle}</div>
               </div>
            ...
            {isPermissionDropdownOpen && (
              <div className="permission-dropdown-menu">
                ...
                {PERMISSION_MODES.map(item => (
                  <div 
                    key={item.id} 
                    className={`permission-dropdown-item ${settings.workspaceMode === item.id ? 'selected' : ''}`}
                    onClick={() => { 
                      handleUpdate('workspaceMode', item.id); 
                      setIsPermissionDropdownOpen(false); 
                    }}
                  >
                    ...
                    {settings.workspaceMode === item.id && (
