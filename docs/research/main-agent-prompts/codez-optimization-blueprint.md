...
TodoItem.verificationEvidence?       structured observed result
TodoCreate.expectedRevision          compare-and-swap guard
TodoCreate.idempotencyKey?           durable retry receipt
TodoUpdate.expectedRevision          compare-and-swap guard
TodoUpdate.reason?                   required for material replanning
TodoUpdate.updates[]                 atomic multi-item patches
TodoArchive.expectedRevision         bounded terminal history
...
1. 同一 session 允许多个真正并行执行的 `in_progress` Todo；排队、阻塞或仅委派的工作仍为 `pending`。
...
10. 模型看到 `TodoCreate`、`TodoUpdate` 与 `TodoArchive`；Get/List 是内部 UI/恢复能力，权威状态和 nextAction 每个 Provider round 自动注入。
11. 一批 patch 在克隆快照上完成后校验最终图和状态，只增加一次 revision、持久化一次、发出一次事件。
