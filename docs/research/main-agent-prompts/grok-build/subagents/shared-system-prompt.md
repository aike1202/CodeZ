# Grok Build 子智能体共享 System Prompt

证据等级：B，源码逐字模板。

来源：`crates/codegen/xai-grok-agent/templates/subagent_prompt.md`。
...
内置 Agent 的 `GENERAL_PURPOSE_PROMPT`、`EXPLORE_PROMPT`、`PLAN_PROMPT` 不位于 `<role-instructions>`：它们存入 `AgentDefinition.prompt_body`，在共享模板渲染后以 `\n\n` 追加。`<role-instructions>` 保存的是可选 role overlay。模板虽然支持 `<persona>`，但当前真实子会话派发把该参数传为 `None`，persona 正文改由独立 `<system-reminder>` 会话项注入。

环境、实际工具名、memory 开关、role 和 persona 在创建或恢复时重新解析，所以没有跨所有运行配置都不变的唯一最终字符串。三个内置 Agent 文件各保存一份参数已固定、条件块已删除、工具名已展开的完整 system prompt 快照。
