# Grok Build 主 Agent 提示词

以下内容逐字来自 `crates/codegen/xai-grok-agent/templates/prompt.md`。`${{ ... }}` 和 `${% ... %}` 是 MiniJinja 变量/条件，不是省略号。
...
默认 `system_prompt_label = "Grok"`。交互模式渲染为 “an interactive CLI tool”；headless/SDK 模式渲染为 “an autonomous agent”。read/edit/monitor 对应工具不存在时，相关示例或整个区块会被移除。
