# Claude Code 主 Agent 的委派提示词

以下是默认非 coordinator、非 fork 路径下 Agent 工具描述的核心正文。Agent 目录可能内联，也可能通过 `agent_listing_delta` 附件注入。
...
{{ Available agent types and the tools they have access to, or an attachment notice }}
...
这条优化提示与“不要过度使用子 Agent”“用户明确要求并行时必须同消息派发”共同生效。不能把它单独解释成固定派发 4 个 Agent；任务必须先能拆成独立、不重叠的工作。

## 主 system 中的 Explore 门控
