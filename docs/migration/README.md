...
- Phase 3：进行中。`WorkspaceRoot`/`SafeWorkspacePath` 已作为私有字段值对象落地，`FileSystem` port 不再接受裸 `Path`；绑定单一 canonical root 的 `NativeFileSystem` 已实现最近存在祖先解析、Windows 大小写身份、内部 symlink 物理折叠、外部 symlink 防逃逸、root/path/父目录/目标 TOCTOU 复检、打开句柄身份校验、有界读取和同目录原子写入。Windows 真实 symlink 逃逸与验证后重定向测试通过；目录树、最近项目、项目识别、Glob/Grep/List/Read、Git/worktree、编辑、附件与进程/PTY 仍未迁移。
...
- `cargo test --workspace --locked`：通过，共 96 项 Rust 单元/集成测试；其中 storage 36 项覆盖路径、原子替换、不可变创建、备份、凭据适配、三类旧凭据迁移、transform、语义引用、重启阶段识别、完成标记、脱敏、幂等与篡改阻断；core/runtime/platform 覆盖进程边界、错误分类、UUID、取消树、路径值对象、Windows 大小写、symlink/TOCTOU、有界读取与原子写入，另有 1 项 `SecretValue` 不可序列化的 compile-fail rustdoc。
