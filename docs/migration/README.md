...
- `cargo test --workspace --locked`：通过，共 91 项 Rust 单元/集成测试；其中 storage 36 项覆盖路径、原子替换、不可变创建、备份、凭据适配、三类旧凭据迁移、transform、语义引用、重启阶段识别、完成标记、脱敏、幂等与篡改阻断；core/runtime/platform 新增 6 项覆盖进程边界、错误分类、UUID 与取消树，另有 1 项 `SecretValue` 不可序列化的 compile-fail rustdoc。
