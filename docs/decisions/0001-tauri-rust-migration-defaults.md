...
| D-03 | 保留产品名 `CodeZ` 和应用 ID `com.codez.desktop`，以维持安装升级身份和系统权限连续性；数据根由 ADR 0007 独立决定。 |
...
- 用户密钥无法安全兼容时进入 `requires_reentry`，不得降级为明文或 Base64。恢复状态只能暴露已映射 credential ID 与脱敏原因；重录写入 OS credential store 后必须重新执行验证/activation，并在显式重启前保持正常应用状态不可用。
...
- Shell parser 已由 [ADR 0004](./0004-rust-shell-parser.md) 冻结同版本 Rust tree-sitter 起点；PowerShell/Bash masks、原生 AST fallback 和失败安全语义必须完整迁移。
- 高频 Tauri 流已由 [ADR 0005](./0005-tauri-stream-backpressure.md) 冻结为有界上游、4 KiB frame、累计 ACK 窗口和显式 cancel；Channel 不作为背压队列。
- Windows x64 资源路径已由 [ADR 0006](./0006-tauri-resource-packaging.md) 冻结：builtin skills 与 `rg` 使用显式 Tauri target map，Rust 只从 `resource_dir` 解析。
- 应用数据路径已由 [ADR 0007](./0007-codez-data-root.md) 冻结：新运行时唯一数据根为 `~/.codez`，Electron `userData` 仅作为只读迁移源。
