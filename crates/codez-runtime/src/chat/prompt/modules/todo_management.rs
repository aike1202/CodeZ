...
- Use Todo for substantial durable work. Create outcome-sized items, not file/tool steps.
...
- Batch related updates atomically. Multiple ready items may be in_progress simultaneously only while genuinely concurrent work is executing; planned, blocked, or queued work is not active. Mark work in_progress immediately before it starts and keep it current.
...
- Allowed transitions are pending -> in_progress/cancelled and in_progress -> completed/pending/cancelled. Cancel only removed or superseded scope, never failed, partial, blocked, or unverified work. Cancellation, dependency changes, and multi-item scope changes require a root reason.
    ...
    fn policy_should_follow_revision_dependency_and_approval_gates() {
        assert!(
            TEXT.contains("expectedRevision")
                ...
                && TEXT.contains("multi-item scope changes require a root reason")
        );
    }
