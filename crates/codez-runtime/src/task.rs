...
impl TaskStore {
    ...
    pub async fn update(
        ...
        let index = next
            ...
            .ok_or_else(|| AppError::not_found("The task was not found"))?;
        let previous = next.tasks[index].clone();
        apply_patch(&mut next.tasks[index], patch);
        validate_task(&next.tasks[index])?;
        if next.tasks[index].status == TaskStatus::InProgress
            && next.tasks.iter().enumerate().any(|(other_index, task)| {
                other_index != index && task.status == TaskStatus::InProgress
            })
        {
...
mod tests {
    ...
    use super::{
        TaskCreateInput, TaskEventSink, TaskSnapshot, TaskStatus, TaskStore, TaskUpdateInput,
    };
