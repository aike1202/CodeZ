...
impl EditTransactionService {
    ...
    pub async fn save_metadata(&self, tx_id: &str) -> Result<(), AppError> {
        ...
        fs::create_dir_all(&tx_dir)
            .await
            .map_err(|e| AppError::internal(format!("Failed to create tx dir: {}", e)))?;
        let metadata_path = tx_dir.join("metadata.json");

        let json = serde_json::to_string(&tx)
            .map_err(|e| AppError::internal(format!("Failed to serialize tx metadata: {}", e)))?;
        let tmp_path = metadata_path.with_extension("tmp");
        fs::write(&tmp_path, json)
            .await
            .map_err(|e| AppError::internal(format!("Failed to write tx metadata: {}", e)))?;
        fs::rename(&tmp_path, &metadata_path)
            .await
            .map_err(|e| AppError::internal(format!("Failed to rename tx metadata: {}", e)))?;
    ...
    pub async fn register_transaction(
        ...
        self.transactions
            .insert(tx_id.to_string(), Arc::new(Mutex::new(tx)));
    ...
    pub async fn backup_file(
        ...
        if let Some(c) = content {
            ...
            fs::write(&backup_path, "")
                .await
                .map_err(|e| AppError::internal(format!("Failed to write empty backup: {}", e)))?;
        }

        tx.backed_up_files
            .insert(file_path.to_path_buf(), backup_path);
        drop(tx);
        self.save_metadata(tx_id).await?;
    ...
    pub async fn record_mutation(
        ...
        if let Ok(content) = fs::read(&file_path).await {
            ...
            tx.expected_post_mutation_sha256
                .insert(file_path.clone(), Some(sha));
        } else {
            tx.expected_post_mutation_sha256
                .insert(file_path.clone(), None);
        }
